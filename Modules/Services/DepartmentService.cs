...
public sealed class DepartmentService : IDepartmentService
{
        ...
        CreateDepartmentRequest request, CancellationToken ct = default)
    {
        if (await _deptRepo.NameExistsAsync(request.Name, ct: ct))
            return ServiceResult<DepartmentResponse>.Fail("NAME_EXISTS",
    ...
    public async Task<ServiceResult<DepartmentResponse>> UpdateAsync(
        string id, UpdateDepartmentRequest request, CancellationToken ct = default)
    {
        ...
        if (request.Name != null && await _deptRepo.NameExistsAsync(request.Name, excludeId: id, ct: ct))
            return ServiceResult<DepartmentResponse>.Fail("NAME_EXISTS",
                $"Phòng ban '{request.Name}' đã tồn tại.");
    ...
    public async Task<ServiceResult<DepartmentResponse>> GetByIdAsync(string id, CancellationToken ct = default)
    {
        var dept = await _deptRepo.FindByIdAsync(id, ct);
        if (dept is null)
            return ServiceResult<DepartmentResponse>.Fail("NOT_FOUND", "Không tìm thấy phòng ban.");
    ...
    public async Task<List<DepartmentResponse>> GetAllAsync(CancellationToken ct = default)
    {
        ...
        return depts
            .Select(d => d.ToResponse(teamCountByDept.GetValueOrDefault(d.id, 0)))
            .ToList();
    }
}
