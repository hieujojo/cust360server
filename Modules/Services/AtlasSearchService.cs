...
public sealed class AtlasSearchService
{
    ...
    private readonly ILogger<AtlasSearchService> _logger;
    ...
    public async Task<CustomerSearchResponse> SearchAsync(string query, int limit = 50, CancellationToken ct = default)
    {
        try
        {
            ...
            var searchStage = new BsonDocument("$search", new BsonDocument
            {
                ...
                ["compound"] = new BsonDocument
                {
                    ["must"] = new BsonArray
                    {
                        ...
                        {
                            ["query"] = query,
                            ["path"] = new BsonArray { "name", "email", "phone", "customerCode" },
            ...
            if (_currentUser.Role == 3 && !string.IsNullOrEmpty(_currentUser.DepartmentId))
            {
                ...
                compound["filter"].AsBsonArray.Add(new BsonDocument("equals", new BsonDocument
                {
                    ["path"] = "departmentId",
            ...
            var addFieldsStage = new BsonDocument("$addFields", new BsonDocument("score", new BsonDocument("$meta", "searchScore")));
            ...
            var responseItems = results.Select(doc => new CustomerSearchResultResponse
            {
                Id = doc["_id"].AsObjectId.ToString(),
                CustomerCode = doc.GetValue("customerCode", "").AsString,
                Name = doc.GetValue("name", "").AsString,
                Status = doc.GetValue("status", "").AsString,
                Email = doc.GetValue("email", BsonNull.Value).IsBsonNull ? null : doc["email"].AsString,
                Phone = doc.GetValue("phone", BsonNull.Value).IsBsonNull ? null : doc["phone"].AsString,
        ...
        {
            // Atlas Search không khả dụng (e.g. chạy local), fallback sang regex
            _logger.LogInformation("Atlas Search không khả dụng, fallback sang Regex search.");
            return await FallbackRegexSearchAsync(query, limit, ct);
        }
    }

    private async Task<CustomerSearchResponse> FallbackRegexSearchAsync(string query, int limit, CancellationToken ct)
    {
        ...
        var filter = Builders<BsonDocument>.Filter.Eq("organizationId", _currentUser.OrganizationId)
