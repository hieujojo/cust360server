...
public sealed class UserService : IUserService
{
        ...
        CancellationToken ct = default)
    {
        ...
        var deptTeamValidation = await ValidateDepartmentAndTeamAsync(request.DepartmentId, request.TeamId, ct);
        ...
        CancellationToken ct = default)
    {
        ...
        if (user.role == Roles.Owner && request.Role.HasValue && request.Role.Value != Roles.Owner)
            return ServiceResult<UserResponse>.Fail("CANNOT_CHANGE_OWNER_ROLE",
        ...
        CancellationToken ct = default)
    {
        ...
        if (user.role == Roles.Owner)
            return ServiceResult.Fail("CANNOT_DEACTIVATE_OWNER", "Không thể vô hiệu hóa tài khoản Owner.");

        if (user.isActive == request.IsActive)
            return ServiceResult.Fail("NO_CHANGE",
        ...
        _logger.LogInformation("User {Action}: {Email} by {Actor}", action, user.email, _currentUser.Email);
        return ServiceResult.Ok();
    }

    // ─── Read ────────────────────────────────────────────────────────────────

    public async Task<ServiceResult<UserResponse>> GetByIdAsync(string userId, CancellationToken ct = default)
    {
        var user = await _userRepo.FindByIdAsync(userId, ct);
        if (user is null)
            return ServiceResult<UserResponse>.Fail("NOT_FOUND", "Không tìm thấy user.");
    ...
    public async Task<PagedResult<UserResponse>> GetPagedAsync(GetUsersRequest request, CancellationToken ct = default)
    {
        ...
        var (items, total) = await _userRepo.FindPagedAsync(
            request.Role, deptFilter, request.IsActive, request.Search,
    ...
    private static ServiceResult ValidateCreateRequest(CreateUserRequest request)
    {
        ...
        if (request.Role == Roles.Owner)
            return ServiceResult.Fail("CANNOT_CREATE_OWNER", "Không thể tạo tài khoản Owner qua API.");

        if (request.Role == Roles.User && string.IsNullOrWhiteSpace(request.DepartmentId))
            return ServiceResult.Fail("DEPT_REQUIRED", "User (Role 3) phải được gán vào một phòng ban.");
        ...
        string? departmentId, string? teamId, CancellationToken ct)
    {
        ...
        if (!string.IsNullOrEmpty(teamId))
        {
            ...
            if (team.departmentId != departmentId)
                return ServiceResult.Fail("TEAM_DEPT_MISMATCH", "Team không thuộc phòng ban được chỉ định.");
        }
    ...
    private static UpdateDefinition<User> BuildUpdateDefinition(UpdateUserRequest request)
    {
        ...
        if (request.TeamId       != null) update = update.Set(x => x.teamId,       request.TeamId);
    ...
    private void SendWelcomeEmailFireAndForget(string email, string displayName, string password)
        => _ = Task.Run(async () =>
            await _emailService.SendAccountCreatedAsync(email, displayName, password, CancellationToken.None));

    private void SendDeactivatedEmailFireAndForget(string email, string displayName)
        => _ = Task.Run(async () =>
            await _emailService.SendAccountDeactivatedAsync(email, displayName, CancellationToken.None));
        ...
        CancellationToken ct = default)
    {
        ...
        _logger.LogInformation("Password changed: {Email}", user.email);
        return ServiceResult.Ok();
    }
        ...
        CancellationToken ct = default)
    {
        ...
        _logger.LogInformation("Password reset: {Email} by {Actor}", user.email, _currentUser.Email);
        return ServiceResult.Ok();
    }

    private void SendPasswordResetEmailFireAndForget(string email, string displayName, string newPassword)
        => _ = Task.Run(async () =>
            await _emailService.SendPasswordResetAsync(email, displayName, newPassword, CancellationToken.None));
}
